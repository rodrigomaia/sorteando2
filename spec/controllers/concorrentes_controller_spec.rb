...
describe ConcorrentesController do
  ...
  describe "GET new" do    
    ...
    it "seta o sorteio encontrado no concorrente" do
      concorrente_double = double('Concorrente')
      Concorrente.stub(:new).and_return(concorrente_double)
      sorteio_double = double('Sorteio')
      Sorteio.stub(:find).and_return(sorteio_double)
      
      concorrente_double.should_receive(:sorteio=).once().with(sorteio_double)
      get :new, :sorteio_id => 1
    end
  end
  ...
  describe "POST create" do    
    before do
      @concorrente_double = double('Concorrente', :id=>1, :sorteio_id=>1)
      Concorrente.stub(:new).and_return(@concorrente_double)
    end
    ...
    context "se concorrente for salvo com sucesso" do
      before do
        @concorrente_double.stub(:save).and_return(true)
      end
    ...
    context "se concorrente não for salvo" do
      before do
        @concorrente_double.stub(:save).and_return(false)
      end
  ...
  describe "DELETE destroy" do    
    before do
      @concorrente_double = double('Concorrente', :id=>1, :sorteio_id=>1)
      Concorrente.stub(:find).and_return(@concorrente_double)
      @concorrente_double.stub(:destroy)
    end
    
    it "deve apagar concorrente informado" do
      @concorrente_double.should_receive(:destroy)
